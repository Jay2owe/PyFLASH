...
Usage:
    ...
    from IF_analysis.plotting import bar_chart_action
...
def get_display_name(name, minimal=False, compact_per=False):
    ...
    if is_per_mm3 and (not minimal) and '/ mm3' not in label:
        label = f"{label} / mm3"
...
def _count_level_processes(experiment, level, factor=None, specificity=None):
    ...
    if level == 'scns':
        return int(summary['SCN'].dropna().nunique()) if 'SCN' in summary.columns else 0
    if level == 'columns':
...
def _save_representative_source(source, verbose=False):
    from IF_analysis.serialization import save_state
...
def _persist_image_edits_and_return(source, fig, marker_names=None, adjustments=None, *,
    ...
    if fig is not None:
        try:
            fig.IF_analysis_saved_image_edits = saved
        except Exception:
            pass
    return fig
...
def _draw_image_panel_roi_outline(ax, source, row, *, black_background=False):
    ...
    scn_name = str(ref_row.get("SCN", "")).strip()
...
def _filter_marker_df_for_context(ctx: Context, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if ctx.animal is not None:
        ...
        elif "SCN" in out.columns and "SCN" in ctx.animal_df.columns:
            allowed_scns = set(_norm(ctx.animal_df["SCN"]).tolist())
            out = out[_norm(out["SCN"]).isin(allowed_scns)]

    # SCN context
    if ctx.scn is not None and "SCN" in out.columns:
        target = str(ctx.scn).strip().casefold()
        out = out[_norm(out["SCN"]) == target]
...
def plot_images(experiment, markers=None,
    ...
    fig.IF_analysis_image_df = work_df
    fig.IF_analysis_save_path = save_path
    fig.IF_analysis_image_adjustments = effective_image_adjustments
    if not show:
        plt.close(fig)
...
def plot_representative_images(source, markers=None,
    ...
    fig.IF_analysis_image_df = filtered_df
    fig.IF_analysis_representative_blocks = blocks
    fig.IF_analysis_save_path = save_path
    fig.IF_analysis_image_adjustments = effective_image_adjustments
    if not show:
        plt.close(fig)
...
def _enrich_df_grouping_columns(df: pd.DataFrame, exp_obj, requested_by: str | None = None):
    ...
    # If SCN lives in index, expose it as a column for merge keys.
    if "SCN" not in out.columns and str(out.index.name) == "SCN":
        out = out.reset_index()
    ...
    if isinstance(summary, pd.DataFrame) and len(summary) > 0:
        ...
        if "SCN" in out.columns and "SCN" in summary_cols:
            scn_cols = ["SCN"] + [c for c in ["AnimalName"] + map_targets if c in summary_cols]
            scn_map = summary[scn_cols].dropna(subset=["SCN"]).drop_duplicates(subset=["SCN"], keep="first")
            out = out.merge(scn_map, on="SCN", how="left", suffixes=("", "__scn"))
            for col in [c for c in ["AnimalName"] + map_targets if f"{c}__scn" in out.columns]:
                if col in out.columns:
                    out[col] = out[col].where(out[col].notna(), out[f"{col}__scn"])
                else:
                    out[col] = out[f"{col}__scn"]
                out.drop(columns=[f"{col}__scn"], inplace=True)
...
def _filter_image_df_for_context(ctx: Context, df: pd.DataFrame) -> pd.DataFrame:
    """Apply condition/factor/animal/SCN filtering to an imported image table."""
    out = df
...
def _location_panel_scn(ctx: Context, df: pd.DataFrame, image_row=None) -> str | None:
    if getattr(ctx, "scn", None) not in (None, ""):
        return str(ctx.scn).strip()
    if isinstance(df, pd.DataFrame) and "SCN" in df.columns:
        scns = [
            str(value).strip()
            for value in df["SCN"].dropna().astype(str).tolist()
            if str(value).strip() != ""
        ]
...
def _location_roi_bounds(exp_obj, scn_name=None, animal_name=None, image_roi_name=None):
    ...
    if normalized_image_roi != "" and "ImageROI" in roi_df.columns:
        ...
        if not narrowed.empty:
            roi_df = narrowed
    if scn_name not in (None, "") and "SCN" in roi_df.columns:
        mask = roi_df["SCN"].astype(str).str.strip().str.casefold() == str(scn_name).strip().casefold()
        narrowed = roi_df[mask]
        if not narrowed.empty:
            roi_df = narrowed
    if roi_df.empty:
...
def _location_roi_dimensions(exp_obj, scn_name=None, animal_name=None):
    ...
    if animal_name not in (None, "") and "AnimalName" in roi_props.columns:
        ...
        if not narrowed.empty:
            roi_props = narrowed
    if scn_name not in (None, "") and "SCN" in roi_props.columns:
        mask = roi_props["SCN"].astype(str).str.strip().str.casefold() == str(scn_name).strip().casefold()
        narrowed = roi_props[mask]
        if not narrowed.empty:
            roi_props = narrowed
    if roi_props.empty:
...
def _location_context_name(ctx: Context, level, display=False):
    ...
    if level_s == "scns":
        return str(ctx.scn or ctx.animal or ctx.condition or "all")
    if level_s == "factors":
        return str(ctx.factor_value or ctx.condition or "all")
    if level_s == "columns":
        return str(ctx.column or "all")
    for value in (ctx.factor_value, ctx.scn, ctx.animal, ctx.condition):
...
def _location_join_rows(ctx: Context, join_by):
    ...
    if join_s == "scns":
        return max(1, int(ctx.num_scns or 0))
    return 1
...
def location_scatter_action(ctx: Context, state: dict,
    ...
    return {'name': ctx.animal or ctx.scn}
...
def plot_mean_bars(experiment, filtered_columns=None,
                   ...
                   specificity=None, comparisons=None,
...
def plot_regressions(experiment, x, y,
                     ...
                     specificity=None, save=True, combine=False,
...
def plot_matrices(experiment, filtered_columns=None,
                  ...
                  marker=None, specificity=None, save=True,
                  column_strings=None, regex_string=None, exclude='',
