...
| `data_cols` | list-like or `None` | `None` | Numeric feature columns for PCA. Alias: `columns`. |
| `data_col_contains`, `data_col_regex`, `data_col_exclude` | string/list filters | `None`, `None`, `None` | Discover feature columns. Aliases: `column_strings`, `regex_string`, `exclude`. |
| `hue_column` | string | `'Condition'` | Summary column used to color points. If missing, all points use one level. |
| `filter_by` | mapping, tuple, list, or `None` | `None` | Row filter before PCA. Alias: `specificity`. |
