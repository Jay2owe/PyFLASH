...
| [`run_linear_model_pipeline`](../functions/run_linear_model_pipeline.md) | `<output_dir or batch.data_path>/Modelling/Linear Models/<run_label>/` | Older modelling-oriented coefficients, summaries, metadata, and manifest. |
...
- `if_exists="skip"` returns the cached `manifest.json` when it exists and marks
  the returned dictionary with `reused=True`. This skip policy applies to the
  modern pipeline helpers, not the older `run_linear_model_pipeline` wrapper.
- `if_exists="error"` raises if the run folder already exists.
- `run_linear_model_pipeline` supports `overwrite`, `version`, and `error` only;
  its `output_dir` is a base directory, and PyFLASH still appends
  `Modelling/Linear Models/<run_label>/`.
